...
if df is not None:
    ...
    # ==========================================
    # 模式五：全真模擬考 (Mock Exam) - 🌟 全新功能！
    # ==========================================
    elif mode == "🏆 全真模擬考 (Mock Exam)":
        st.header("🏆 全真模擬考 (拼寫測試)")
        ...
        if not st.session_state.mock_active:
            ...
            if curr_q < total_q:
                ...
                st.subheader(f"請拼出 **{current_bac}** 的用藥")
                st.caption(f"💡 分類提示：{row['分類']}")
                
                st.info(f"**🧩 格式密碼提示：**\n\n首選藥： `{generate_hint(row['首選藥'])}`\n\n替代藥： `{generate_hint(row['替代藥'])}`")
                
                col1, col2 = st.columns(2)
                with col1: st.text_input("✍️ 首選藥", key=f"mock_doc_{curr_q}")
                with col2: st.text_input("✍️ 替代藥", key=f"mock_alt_{curr_q}")
